### Development verification (devenv tasks)

Use `devenv tasks run <task-name>` to run checks in the project environment.

- `verify:lint` - runs `elm-format --validate src app`
- `verify:review` - runs `elm-review`
- `verify:build` - runs `npm run build`

Suggested routine before/after changes:
